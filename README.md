...
2. Модуль комплексного числа: Модуль комплексного числа a = a1+a2i вычисляется по формуле: |a| = sqrt(a1<sup>2</sup> + a2<sup>2</sup>
...
5. Вычисление произведения всех модулей:
 - Перебираем все элементы массива и вычисляем произведение их модулей. Модуль вычисляется по формуле:
    $`\sqrt(realPart[i][j]^2+imagPart[i][j]^2)`$
